...
function getRectangleString(width, height) {
  ...
  const stringFabric = (firstChar, lastChar, spacer, length) => (
    `${firstChar}${spacer.repeat(length - 2)}${lastChar}\n`
  );

  for (let h = 0; h < height; h += 1) {
    if (h === 0) {
      resultString += stringFabric('┌', '┐', '─', width);
    } else if (h === height - 1) {
      resultString += stringFabric('└', '┘', '─', width);
    } else {
      resultString += stringFabric('│', '│', ' ', width);
